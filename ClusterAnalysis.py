...
# To Do list:
# 1. What is the salt atom size in this case ?  0.5
# 2. How to dermine the cut off distance generally, and dist_factor ?  use the same
# 4. Read dump.py, make sure the style is consistent with each other.  
# 7. polish the code.
...
num_pan = int(sys.argv[4])
charge_ratio = float(sys.argv[5])
...
class ClusterAnalysis:
    ...
    neighborListChain : 2d - ndarray, dtype = int
        ...
        note that the neighborList here is incomplete because we only
    ...
    def __init__(self, ifile, lenPa, lenPc, numPa, chargeRatio, saltCon):
        """
        Constructor.
        """
        ...
        self.numPcBeads = int(self.chargeRatio * self.numPaBeads)
        self.numPc = self.numPcBeads / self.lenPc
...
# Start to write File: PDB file, dump file and a summary file
    ...
    def writeDumpFile(self):
        ...
        """
        dirName = "./ClusterAnalysisPE_la{0}_lc{1}_npa_{2}_cf_{3}_s{4}_x{5}/".\
                  format(self.lenPa, self.lenPc, self.numPa, self.chargeFraction,\
                         self.saltCon, self.chargeRatio)
...
if __name__ == "__main__":
    dirName = "./ClusterAnalysisPE_la{0}_lc{1}_npa_{2}_cf_0.5_s{3}_x{4}/".\
                  format(len_pan, len_pcat, num_pan, salt_concentration,\
                         charge_ratio)
    if not os.path.exists(dirName):
        os.makedirs(dirName)
    ...
    for i in range(1):
        ...
        model = ClusterAnalysis(filename, len_pan, len_pcat, num_pan, charge_ratio, salt_concentration)
        model.readFile(i)
        start = time.time()
        model.analysis()
        model.moveCluster()
